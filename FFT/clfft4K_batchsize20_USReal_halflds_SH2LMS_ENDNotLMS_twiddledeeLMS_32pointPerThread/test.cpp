...
int main(int argc , char *argv[])
{
	...
	/**************************计算误差**************************/
	float FFT_Result_Real[4*1024];
	float FFT_Result_Img[4*1024];
	...
	ifstream in_Real("CLFFT4K_Result_Real.txt");
	for(int i=0;i<4*1024;i++)
	{
		in_Real>>FFT_Result_Real[i];
	}

	ifstream in_Img("CLFFT4K_Result_Img.txt");
	for(int i=0;i<4*1024;i++)
	{
	...
	{
		FFT_err_Real[i]=A1[i*2]-FFT_Result_Real[i%(4*1024)];
		...
		if(FFT_Result_Real[i%(4*1024)]!=0)
		{
			FFT_err_Real[i]=abs(FFT_err_Real[i])/abs(FFT_Result_Real[i%(4*1024)])*100;//计算相对误差
		}

		FFT_err_Img[i]=A1[i*2+1]-FFT_Result_Img[i%(4*1024)];
		...
		if(FFT_Result_Img[i%(4*1024)]!=0)
		{
			FFT_err_Img[i]=abs(FFT_err_Img[i])/abs(FFT_Result_Img[i%(4*1024)])*100;
		}		
