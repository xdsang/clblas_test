...
int main(int argc , char *argv[])
{
	...
	size_t global_work_size_kernel1[1] = { 128 * 1* batchsize};//一个4K的FFT kernel1有4个group 每个group有128个线程
	size_t local_work_size_kernel1[1] = { 128 };

	size_t global_work_size_kernel2[1] = { 128 * 1* batchsize};
	size_t local_work_size_kernel2[1] = { 128 };
    ...
    for (int j = 0; j<iteration; j++)
    {
		for (i = 0; i < num_platforms; ++i)
		{
			for (int dev_id = 0; dev_id < num_devices[i]; ++dev_id)
			{
				...
				//kernel2,output of kernel1 as input of kernel2
	...
	/**************************计算误差**************************/
	float FFT_Result_Real[8*1024];
	float FFT_Result_Img[8*1024];
	float *FFT_err_Real,*FFT_err_Img;
	
	FFT_err_Real = (float *)malloc(dataSize * sizeof(float));
	FFT_err_Img = (float *)malloc(dataSize * sizeof(float));//相对误差=abs{绝对误差-目标值(matlab计算结果) }/目标值
	...
	float MRE_Real=0;
	float MRE_Img=0; //平均相对误差

	ifstream in_Real("CLFFT8K_Result_Real.txt");
	for(int i=0;i<8*1024;i++)
	{
		in_Real>>FFT_Result_Real[i];
	}

	ifstream in_Img("CLFFT8K_Result_Img.txt");
	for(int i=0;i<8*1024;i++)
	{
	...
	{
		FFT_err_Real[i]=abs(A1[i*2]-FFT_Result_Real[i%(8*1024)]);
		if(max_dist_Real<FFT_err_Real[i])
		{
			max_dist_Real=FFT_err_Real[i];
			max_dist_Real_ID=i;
		}
		if(FFT_Result_Real[i%(8*1024)]!=0)
		{
			FFT_err_Real[i]=abs(FFT_err_Real[i])/abs(FFT_Result_Real[i%(8*1024)])*100;//计算相对误差
		}

		FFT_err_Img[i]=abs(A1[i*2+1]-FFT_Result_Img[i%(8*1024)]);
		...
		if(FFT_Result_Img[i%(8*1024)]!=0)
		{
			FFT_err_Img[i]=abs(FFT_err_Img[i])/abs(FFT_Result_Img[i%(8*1024)])*100;
		}		
		MRE_Real=FFT_err_Real[i]+MRE_Real;
		MRE_Img=FFT_err_Img[i]+MRE_Img;//所有相对误差都累加
	}
	...
	else
	{
		for(int i=0;i<20;i++)
		{
			printf("FFT_err_Img %f FFT_err_Real %f\n",FFT_err_Img,FFT_err_Real);
		}
		printf("偏差较大，结果也许不对\n");
		printf("平均相对误差:MRE_Real=%f% MRE_Img=%f%(若大于0.05，认为偏差较大)\n",MRE_Real,MRE_Img);
		printf("误差最大点的实部ID %d 误差最大点的实部误差 %f\n误差最大点的虚部ID %d 误差最大点的虚部误差 %f\n",max_dist_Real_ID,max_dist_Real,max_dist_Img_ID,max_dist_Img);
		/******************************将最大误差的点的值和误差都打印出来******************************/
		printf("--------------------------------------------:\n");
