...
void InitialMatrix(FloatComplex *Mat, int rows, int cols, int batch)
{
...
void ShowMatrix(FloatComplex *Mat, int lda, int rows, int cols, int batch)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols * batch; j++)
        {
            printf("%f + %fj  ", Mat[i + j * lda].x, Mat[i + j * lda].y);
        }
        printf("\n");
    }
    printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    FloatComplex *A, *B, *C;
    A = (FloatComplex *)malloc(A_size * batch * sizeof(*A));
    B = (FloatComplex *)malloc(B_size * batch * sizeof(*B));
    C = (FloatComplex *)malloc(C_size * batch * sizeof(*C));
    if (!A || !B || !C)
    {
    ...
    for (size_t j = 0; j < batch; j++)
    {
        for (int i = 0; i < num_platforms; ++i)
        {
            for (int dev_id = 0; dev_id < num_devices[i]; ++dev_id)
            {
                ...
                err = clblasCgemm( clblasColumnMajor, clblasNoTrans, clblasNoTrans,
                                  A_ROW, B_COL, A_COL,
    ...
    FloatComplex *C_result = (FloatComplex *)malloc(C_size * batch * sizeof(*C_result));
    float alpha_cblas = 1.0f;
    float beta_cblas = 0.0f;
    for (size_t i = 0; i < batch; i++)
    {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, A_ROW, B_COL, A_COL, &alpha_cblas,
                    A + i * A_size, A_ROW, B + i * B_size, B_ROW, &beta_cblas, C_result + i * C_size, A_ROW);
    }
    ...
    {
        e.x = abs(C_result[i].x - C[i].x) / (C_result[i].x + 1e-09);
        e.y = abs(C_result[i].y - C[i].y) / (C_result[i].y + 1e-09);
        if (e.x > 1e-05 || e.y > 1e-05)
        {
            printf("MatrixMul verify fail!please check it!\n\n");
            printf("Error result: C[%d] = %f + %fj, C_result[%d] = %f + %fj\n\n", i, C[i].x, C[i].y, i, C_result[i].x, C_result[i].y);
            return -1;
        }
    }

    printf("\nMatrixMul verify pass! \n");
    printf("\nMatrix size: %d*%d, batch: %d, kernel time: %f ms \n", N, N, batch, ktime * (1e03));
    printf("\n");
